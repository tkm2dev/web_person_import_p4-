...
```
แถว 1-2    ชื่อเรื่อง | อัปเดตล่าสุด | [☑ ปุ่มดึงข้อมูลล่าสุด] | สถานะ
...
ช่อง **D2** เป็น checkbox — ติ๊กแล้วมันจะดึงข้อมูลล่าสุดทันที เคลียร์ติ๊กเอง แล้วเขียนผลลง E2

ปุ่มนี้ใช้ sync แบบ **incremental** (~10 วิ) ไม่ใช่ force rebuild — กดบ่อยก็ไม่กินโควตา

> Apps Script สร้างปุ่มแบบ "รูปวาด + กำหนดสคริปต์" ด้วยโค้ดไม่ได้
> จึงใช้ checkbox + installable `onEdit` trigger แทน
...
| `onEditMaster` | เมื่อแก้ไฟล์ ภ.4 | ดักปุ่ม D2 |
