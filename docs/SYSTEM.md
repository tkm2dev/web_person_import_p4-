...
### ปุ่มดึงข้อมูลบนแท็บ ภาพรวม

แถว 2: `C2` เป็นป้าย **"↻ ดึงข้อมูลล่าสุด"** `D2` เป็น checkbox `E2` เป็นสถานะ

ติ๊ก `D2` → `onEditMaster` ดักได้ → เคลียร์ติ๊กทันที → ขึ้น "กำลังดึงข้อมูล..." → เขียนผลลัพธ์ลง `E2`

ใช้ sync แบบ **incremental** (~10 วิ) ไม่ใช่ force rebuild — กดบ่อยก็ไม่กินโควตา
ถ้าอยาก rebuild เต็ม ใช้เมนู `ภ.4 > รวมข้อมูลเดี๋ยวนี้`

> **ทำไมไม่ใช้ปุ่มรูปที่กดได้จริง**
>
> `OverGridImage.assignScript()` วางรูปแล้วผูกฟังก์ชันได้ก็จริง แต่ตอนคลิก Sheets
> จะหาฟังก์ชันจาก **สคริปต์ที่ผูกกับสเปรดชีตนั้น (container-bound)** เท่านั้น
> โปรเจกต์นี้เป็น standalone จึงขึ้น `ไม่พบฟังก์ชันของสคริปต์`
>
> ย้ายไป container-bound ก็ทำได้ แต่ `makeCopy()` จะพ่วงโค้ดทั้งระบบไปกับ
> ไฟล์ที่ส่งส่วนกลาง (`exportForCentral`) และชุดสำรองรายวันทุกชุด (`snapshotMaster`) — ไม่คุ้ม
...
| `onEditMaster` | เมื่อแก้ไฟล์ ภ.4 | ดัก checkbox `D2` บนแท็บ ภาพรวม |
| `onOpenMaster` | เมื่อเปิดไฟล์ ภ.4 | สร้างเมนู "ภ.4" |
| `snapshotMaster` | ทุกวัน ตี 1 | สำรองไฟล์ ภ.4 เก็บ 14 ชุด |

`onEditMaster` ต้องเป็น **installable trigger** ไม่ใช่ simple `onEdit` เพราะต้องใช้สิทธิ์ Drive/Lock
...
| `setProvinceFolderId(id)` † | เก็บ id โฟลเดอร์ลง Script Properties (รับ id หรือ URL เต็ม) |
...
† ฟังก์ชันที่มีเครื่องหมายนี้ **กด "เรียกใช้" จากตัวแก้ไขไม่ได้** เพราะรับพารามิเตอร์
ตัวแก้ไขจะส่ง `undefined` เข้าไปแล้วโยน error ใช้ พร็อพเพอร์ตี้ของสคริปต์ หรือเมนูใน
ไฟล์ ภ.4 แทน (`setProvinceFolderId` มีเมนูให้ `setProvinceIds` ต้องเรียกจากโค้ด)
...
ตั้งเขตเวลาเป็น `(GMT+07:00) กรุงเทพ` ที่ `⚙️ การตั้งค่าโปรเจกต์`

**บอกสคริปต์ว่าโฟลเดอร์ไฟล์จังหวัดอยู่ที่ไหน** — ในหน้าเดียวกัน เลื่อนลงไป
`พร็อพเพอร์ตี้ของสคริปต์` → เพิ่ม

```
PROVINCE_FOLDER_ID = <id โฟลเดอร์จากขั้นที่ 2>
```

> ⚠️ อย่าใส่ค่านี้ใน `CONFIG.PROVINCE_FOLDER_ID` ถ้าซอร์สขึ้น repo
>
> และ **`setProvinceFolderId()` กด "เรียกใช้" จากตัวแก้ไขตรงๆ ไม่ได้** — ตัวแก้ไข
> รันฟังก์ชันที่รับพารามิเตอร์ไม่ได้ จะได้ `undefined` แล้วโยน error
...
apps_script/Code.gs              ระบบทั้งหมด ไฟล์เดียว (~1,600 บรรทัด)
